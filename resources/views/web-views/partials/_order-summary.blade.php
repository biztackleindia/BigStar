...
<aside class="col-lg-4 pt-4 pt-lg-0">
    <div class="cart_total">
        ...
        @php($is_unit_weight=0)
        @php($unit_weight_total=0)
        ...
        @if($cart->count() > 0)
            @foreach($cart as $key => $cartItem)
               ...
               $weight=\App\Model\Product::where(['id'=>$cartItem['product_id']])->first();
               if($weight->unit == 'gms' || $weight->unit == 'kg'){
               $is_unit_weight=1;
               if($weight->unit_weight>0){
                ...
                $unit_weight_total+=$covert_kg*$cartItem['quantity'];
                }
            ...
            //    calculation shipping charge for third party api
            ...
            if($address_id>0){
            $shipping_address = \App\Model\ShippingAddress::where('id',$address_id)->first();
            $ship_total_weight = $unit_weight_total*1000;
            $ship_weight =  \App\CPU\Helpers::WeightWithShippingCost($shipping_address->zip,$ship_total_weight);
            if($ship_weight!=0){
    ...
    @if($is_unit_weight==1)
        <div class="d-flex justify-content-between">
            <span class="cart_title" style="font-size: 11px;">{{\App\CPU\translate('total')}} {{\App\CPU\translate('weight')}}</span>
            <span class="cart_value" style="font-size: 11px;"> {{$unit_weight_total}} Kg</span>
        </div>
    @endif
