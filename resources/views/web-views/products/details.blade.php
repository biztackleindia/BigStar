...
@push('css_or_js')
    ...
    line-height: 1.5;
        ...
        .jm-btn{
            color: #ffffff;
    background-color: rgb(237 29 70);
    ...
    margin-top: 10px;
        ...
        .flash_deal_product  .discout-box{
            top: auto !important;
        }
        #pin_error{
       margin-top: 2px;
...
@section('content')
    ...
    <div class="container mt-4 rtl" style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
        ...
        <div class="row" style="direction: ltr">
                        ...
                        <div class="details">
                            ...
                            @else
                                ...
                                <span
                                    ...
                                    {{\App\CPU\Helpers::get_member_price_range($product) }}
                                </span>
                                @endif
                                @else
                                <span
                                ...
                                @if($product->unit == 'gms' || $product->unit == 'kg')
                                <span class="{{Session::get('direction') === "rtl" ? 'mr-2' : 'ml-2'}}"
                                    style="font-size: 12px;font-weight:400">
                                    <span>{{\App\CPU\translate('weight')}} : </span>
                                    <span>{{$product->unit_weight}} {{$product->unit}}</span>
                                </span>
                                @endif
                                <span class="{{Session::get('direction') === "rtl" ? 'mr-2' : 'ml-2'}}"
                                    style="font-size: 12px;font-weight:400">
                            ...
                            <div class="mt-3">
                                <span class="mb-2" style="font-size: 20px;font-weight:700;">
                                {{\App\CPU\translate('Check_Estimated_Delivery')}}</span>

                                <form  style="display: flex;">
                                <input type="tel" class="inp_text" name="rel_pincode" id="prod_rel_pincode" 
                                placeholder="{{\App\CPU\translate('Enter_pincode')}}"
                                autocomplete="off" maxlength="6">
                                <button type="button" id="prod_pin_form" class="jm-btn secondary medium">Apply</button>
                                </form>
                                <div id="pin_sucess"></div>
            ...
            <div class="col-md-3 ">
            @php($category_data=json_decode($product['category_ids']))
            
            @if (count(($category_data)) ==2 )
            @foreach($category_data as $category)
            @php($product_category = App\Model\Category::where('parent_id', '!=', 0)->where('id',$category->id)->latest()->get())
...
@push('script')
    ...
    <script>
        ...
        $('#prod_pin_form').on('click', function () {
           
           if ($('#prod_rel_pincode').val() != '') {
              console.log($('#prod_rel_pincode').val());
                   $.ajaxSetup({
                       headers: {
                           'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
                       }
                   ...
                   $.ajax({
                       type: "post",
                       url: '{{route('avilable_pincode')}}',
                       data: {
                           rel_pincode: $('#prod_rel_pincode').val(),
                       },
                       success: function (respons) {
                        console.log(respons);
                        if (respons) {
                            $('#pin_sucess').html('Delivery available for your pincode');
                            $('#pin_error').html('');
                        } else {
                            $('#pin_sucess').html('');
                            $('#pin_error').html('Not available for your pincode');
                        }
                       }
                   });
           }
