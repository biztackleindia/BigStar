...
<div class="container py-4 py-lg-5 my-4"
    ...
    <div class="row justify-content-center">
        <div class="col-md-6">
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-6">
                                <div class="form-group">
                                    <label for="reg-phone">{{\App\CPU\translate('phone_number')}}*
                                        <small class="">( {{\App\CPU\translate('country_code_is_must')}}
                                            )</small></label>
                           ...
                           <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <div class="password-toggle">
                                    <input class="form-control" name="con_password" id="confirm_password"
                                        type="password"
                                        style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};"
                                        placeholder="{{\App\CPU\translate('minimum_8_characters_long')}}" required>
                                    <label class="password-toggle-btn">
                         ...
                         @php($member_settings=\App\CPU\Helpers::get_business_settings('member_settings'))
                         @if($member_settings==1)
                         <div class="col-sm-6">
                                <div class="form-group">
                                
                                    <label for="reg-fn">{{\App\CPU\translate('already a member (optional)')}}</label>
                                    <div style="display:flex">
                                    <input class="form-control" value="MID" type="text" disabled style="width: 45px; padding: 0px 8px 0px;    border-top-right-radius: 0px; border-bottom-right-radius: 0px;text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};"
                                        >
                                        <input class="form-control" value="{{old('member_id')}}" type="text" name="member_id"
                                        style="border-top-left-radius: 0px; border-bottom-left-radius: 0px;text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};"
                                        >
                                    </div>
                                    <div class="invalid-feedback">
                                        {{\App\CPU\translate('Please enter your member id')}}!</div>
                                </div>
                            </div>
                            @endif
                            <div class="col-sm-6">
                                <div class="form-group">
                                    ...
                                    <label for="reg-fn">{{\App\CPU\translate('referral code')}} 
                                        <span style=" font-size: 12px;">({{\App\CPU\translate('optional')}})</span></label>
                                    <div style="display:flex">
                                         <input class="form-control" value="{{old('referral_code')}}" type="text" name="referral_code"
                                        style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};"
                                        >
                                    </div>
                                    <div class="invalid-feedback">
                                        {{\App\CPU\translate('Please enter your referral code')}}!</div>
                                </div>
                            </div>
                        </div>
            ...
            <div class="mx-1">
                <div style="    text-align: center;">{{\App\CPU\translate('already_have_an_account?')}}</div>
                <div style="width:100%;text-align: center; margin-top: 15px;"> 
                    <a class="btn btn-outline-primary" style="width:80%" href="{{route('customer.auth.login')}}">
...
$('#si-password, #confirm_password').on('keyup', function() {
    if ($('#si-password').val().length>7) {
       
        if ($('#si-password').val() == $('#confirm_password').val()) {
            $('#message').html('');
            
        } else {
            if ($('#confirm_password').val().length>3) {
            $('#message').html('Passwords do not match!').css('color', 'red');
            }
            
        }
        $('#character_message').html('');
    } else {
        $('#message').html('').css('color', 'green');
        $('#character_message').html('Minimum 8 characters long').css('color', 'red');
