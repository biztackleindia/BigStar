                    ...
                    <div class="card-body" style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
                        <form action="{{route('admin.category.update',[$category['id']])}}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                    ...
                                    @foreach(json_decode($language) as $lang)
                                        ...
                                        <div class="form-group {{$lang != $default_lang ? 'd-none':''}} lang_form"
                                            ...
                                            <label class="input-label">{{\App\CPU\translate('name')}}
                                                ({{strtoupper($lang)}}) {{$lang == $default_lang? '(*)':''}}</label>
                                ...
                                @if($category['parent_id']==0)
                                    <div class="col-12 col-md-4 from_part_2">
                                        <label>{{\App\CPU\translate('image')}}</label><small style="color: red">
                                            ( {{\App\CPU\translate('ratio')}} 1:1 , {{ \App\CPU\translate('size')}} 500 x 500 )</small>
                                ...
                                <!-- video upload only sub categorys -->
                                @if($category['position']==1)

                                <div class="col-12 col-md-6 mt-3 " style="text-align: left">
                                <label class="input-label" style="margin-top: -25px;">{{\App\CPU\translate('video')}}  </label>
                                        <input type="file" name="sub_categoty_video" id="customFileUploadvideo"
                                               class="custom-file-input">
                                        <label class="custom-file-label" style="  width: 97%;  margin: 0px 15px;"
                                               for="customFileUploadvideo">{{\App\CPU\translate('choose')}} {{\App\CPU\translate('file')}}</label>
                                </div>
...
@push('script')
    ...
    <script>
        ...
        $("#customFileUploadvideo").change(function () {
