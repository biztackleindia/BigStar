...
@section('title', \App\CPU\translate('Member List'))
    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <div class="row align-items-center mb-3">
                <div class="col-sm">
                    <h1 class="page-header-title">{{\App\CPU\translate('member')}}
            ...
            <span class="hs-nav-scroller-arrow-prev" style="display: none;">
              ...
              <a class="hs-nav-scroller-arrow-link" href="javascript:;">
                ...
                <ul class="nav nav-tabs page-header-tabs">
                    <li class="nav-item">
                        <a class="nav-link active" href="#">{{\App\CPU\translate('member')}} {{\App\CPU\translate('List')}} </a>
                    </li>
                </ul>
                <!-- End Nav -->
            </div>
                ...
                <div class="flex-between row justify-content-between align-items-center flex-grow-1 mx-1">
                            <div>
                                <div class="flex-start">
                                    <div><h5>{{ \App\CPU\translate('member')}} {{ \App\CPU\translate('Table')}}</h5></div>
            ...
            <div class="table-responsive datatable-custom">
                <table
                    ...
                    <tr>
                        ...
                        <th>{{\App\CPU\translate('member_id')}}</th>
                        <th>{{\App\CPU\translate('member_approval')}}</th>
                    ...
                    @foreach($customers as $key=>$customer)
                        <tr class="">
                            ...
                            <td>
                            <!-- <label class="switch"> -->
                                @if($customer['member_approval']==0)
                              <button type="button" class="approve_member" id="cust_{{$customer['id']}}" > 
                                {{\App\CPU\translate('pending')}}
                               </button>
                               @elseif($customer['member_approval']==1)
                               <button type="button" class="reject_member" id="reject_{{$customer['id']}}" > 
                                {{\App\CPU\translate('approve')}}
                               </button>
                               @elseif($customer['member_approval']==2)
                               <button type="button" class="reject_btn"  id="approv_{{$customer['id']}}"> 
                            ...
                            <td>
                                ...
                                <a  title="{{\App\CPU\translate('delete')}}"
                                    ...
                                    onclick="form_alert('customer-{{$customer['id']}}','Want to delete this member ?')">
...
@push('script_2')
    ...
    <script>
         $(document).on('click', '.approve_member', function () {
            ...
            $.ajax({
                url: "{{route('admin.customer.approve-status')}}",
                method: 'POST',
                data: {
                    id: cust_id,
                },
                success: function () {
                    toastr.success('{{\App\CPU\translate('Member Approve successfully')}}');
                    location.reload();
                }
            });
        });
        $(document).on('click', '.reject_member', function () {
            ...
            $.ajax({
                url: "{{route('admin.customer.reject-status')}}",
                method: 'POST',
                data: {
                    id: cust_id,
                },
                success: function () {
                    toastr.success('{{\App\CPU\translate('Member Rejected successfully')}}');
                    location.reload();
                }
            });
        });
