    ...
    <section class="col-lg-8">
        ...
        <div class="cart_information mb-3">
                ...
                <table class="table table-borderless table-thead-bordered table-nowrap table-align-middle card-table"
                    ...
                    <thead class="">
                        <tr class="">
                            ...
                            <th class="font-weight-bold" style="width: 45%;">{{\App\CPU\translate('product_details')}}
                            </th>
                            @else
                            <th class="font-weight-bold" style="width: 45%;">{{\App\CPU\translate('product_details')}}
                            </th>
                            @endif
                            <th class="font-weight-bold" style="width: 15%; min-width: 84px;">{{\App\CPU\translate('unit_price')}}</th>
                            <th class="font-weight-bold" style="width: 15%; text-align: center;">{{\App\CPU\translate('qty')}}</th>
                            <th class="font-weight-bold" style="width: 15%; min-width: 98px;">{{\App\CPU\translate('price')}}</th>
                            @if ( $shipping_type != 'order_wise')
                            <th class="font-weight-bold" style="width: 15%;">{{\App\CPU\translate('shipping_cost')}}
                            </th>
                            @endif
                    ...
                    @php($weight=\App\Model\Product::where(['id'=>$cartItem['product_id']])->first())
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex">
                                    ...
                                    <div class="ml-2 text-break" style="width:60%;">
                                        <a href="{{route('product',$cartItem['slug'])}}">{{$cartItem['name']}}</a><br>
                                        @if($weight->unit == 'gms' || $weight->unit == 'kg')
                                       
                                       <span class="unit_weight"> (Weight :{{$weight->unit_weight}} {{$weight->unit}})</span>
                                        @endif
            ...
            <td>
                ...
                <div class="d-flex justify-content-center align-items-center quantity_box"
                    style="width: 160px;color: {{$web_config['primary_color']}}">
                    <span class="input-group-btn" style="">
                        <button class="btn btn-minus " id="minus{{$cartItem['id']}}" type="button" data-type="minus" data-field="quantity"
                            style="padding: 10px;color: {{$web_config['primary_color']}}">
                            -
                        </button>
                    </span>
                    <input type="text" name="quantity" id="input{{$cartItem['id']}}"  class="form-control input-number text-center cart-qty-field quantity"
                        value="{{$cartItem['quantity']}}"  min="1" max="100"
                        style="padding: 0px !important;width: 40%;height: 25px;">
                    <span class="input-group-btn">
                        <button class="btn btn-plus" id="plus_{{$cartItem['id']}}" type="button" data-type="plus" data-field="quantity"
                            style="padding: 10px;color: {{$web_config['primary_color']}}">
                            +
                        </button>
                    </span>
