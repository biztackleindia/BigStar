...
class WebController extends Controller
{
    ...
    public function checkout_details(Request $request)
    {
        ...
        foreach($carts as $cart)
        {
            ...
            if($shipping_type == 'order_wise' && $shippingMethod != 'third_party_shipping'){
    ...
    public function checkout_payment()
    {
        ...
        if (session()->has('address_id') && count($cart_group_ids) > 0) {
            ...
            // $pincode = Pincode::where('pincode', $shipping_address->zip)
            // ->orderBy('created_at', 'DESC')
            // ->get();
            $pincode = Pincode::Where('from_pincode','<=', $shipping_address->zip)
            ->Where('to_pincode','>=', $shipping_address->zip)
            ->orderBy('created_at', 'DESC')
            ->get();

            return view('web-views.checkout-payment', compact('pincode'));
        }
    ...
    //     $amt = CartManager::cart_total_applied_discount(CartManager::get_cart());
    //     $payment = [
    //         // "amount" => round(1,2),
    //         "amount" => round($amt),
    //         "description" =>  'Hello '. $f_name .'. Please pay and confirm your order.',
    //         "currency" => 'BHD',
    //         "customer" => [
    //             "first_name" => $f_name,
    //             "email" => $email,
    ...
    //             "authorization: Bearer sk_test_zQkLR4NwWOVqI95gKHdsSCTh",
    ...
    public function PincodeChecking(Request $request)
    {
            ...
            $Pincode = Pincode::Where('from_pincode','<=', $request->rel_pincode)
            ->Where('to_pincode','>=', $request->rel_pincode)
            ->orderBy('created_at', 'DESC')
            ->first();
    ...
    {
        User::where(['id' => auth('customer')->id()])->update(['member_approval_status' => 1]);
        return response()->json([], 200);
    }
    // public function WeightWithShippingCost(Request $request)
    // {
    //     $client = new Client(); 
