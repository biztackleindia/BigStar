...
class UserProfileController extends Controller
{
    ...
    public function address_store(Request $request)
    {
        $address = [
            ...
            'zip' => $request->zip,
            'phone' => $request->phone,
            'is_billing' =>$request->is_billing ? $request->is_billing :0,
    ...
    {
        // $Pincode = Pincode::where('pincode', $request->zip)
        $Pincode = Pincode::Where('from_pincode','<=', $request->zip)
        ->Where('to_pincode','>=', $request->zip)
        ->orderBy('created_at', 'DESC')
        ->count();
        if ($Pincode==0 ) {
            Toastr::error(translate('Delivery not available for your pincode!'));
            return back();
        }
        $updateAddress = [
            'contact_person_name' => $request->name,
    ...
    public function order_cancel($id)
    {
        ...
        // if ($order['payment_method'] == 'cash_on_delivery' && $order['order_status'] == 'pending') {
            ...
            // / reward poins cancel for order
            $cust_id =$order['customer_id'];
            $order_reward = OrderManager::Product_wise_reward_Points($cust_id,$id);
            $user = User::find($cust_id);
            $reward_points=$user->reward_points ? $user->reward_points : 0;
            if($reward_points >= $order_reward){
    ...
    public function store_refund(Request $request)
    {
        ...
        // / reward poins return for order
            $cust_id =auth('customer')->id();
            $order_reward = OrderManager::Product_wise_reward_Points($cust_id,$order_details->order_id);
            $user = User::find($cust_id);
            $reward_points=$user->reward_points ? $user->reward_points : 0;
            if($reward_points >= $order_reward){
                $reward_points=$reward_points - $order_reward;
