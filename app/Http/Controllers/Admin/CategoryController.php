...
class CategoryController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        if ($request->has('sub_categoty_video')) {
            $filenameWithExt= $request->file('sub_categoty_video')->getClientOriginalName();
            $filename = pathinfo($filenameWithExt, PATHINFO_FILENAME);
            $extension = $request->file('sub_categoty_video')->getClientOriginalExtension();
            $fileNameToStore = $filename. '_'.time().'.'.$extension;
            $path = $request->file('sub_categoty_video')->storeAs('public/videos',$fileNameToStore);
        ...
        $category->slug = Str::slug($request->name[array_search('en', $request->lang)]);
        if ($request->image) {
            $category->icon = ImageManager::update('category/', $request->image, 'png', $request->file('image'));
        }
        if ($request->sub_categoty_video) {
            $category->sub_categoty_video = $fileNameToStore;
        }
        $category->priority = $request->priority;
        $category->save();
    ...
    public function delete(Request $request)
    {
        ...
        if (!empty($categories)) {
            foreach ($categories as $category) {
                ...
                $translation = Translation::where('translationable_type','App\Model\Category')->where('translationable_id',$category->id);
                $translation->delete();
                Category::destroy($category->id);
            }
        }
        $translation = Translation::where('translationable_type','App\Model\Category')->where('translationable_id',$request->id);
        $translation->delete();
