...
class DealController extends Controller
{
    ...
    public function status_update(Request $request)
    {
        ...
        FlashDeal::where(['id' => $request['id']])->update(['status' => $request['status']]);
    ...
    {
        FlashDeal::where(['id' => $request['id']])->update(['featured' => $request['featured']]);
        return response()->json([
            'success' => 1
        ], 200);
    }
    ...
    public function feature_index(Request $request)
    {
        ...
        if ($request->has('search')) {
            $key = explode(' ', $request['search']);
            $flash_deals = FlashDeal::where('deal_type', 'feature_deal')->where(function ($q) use ($key) {
                foreach ($key as $value) {
                    $q->Where('title', 'like', "%{$value}%");
                }
            });
    ...
    public function day_status_update(Request $request)
    {
        ...
        DealOfTheDay::where(['id' => $request['id']])->update(['status' => $request['status']]);
        return response()->json([
            'success' => 1
        ], 200);
    }
