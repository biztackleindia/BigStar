...
class OrderController extends Controller
{
    ...
    public function details($id)
    {
        ...
        $linked_orders = Order::where(['order_group_id' => $order['order_group_id']])->whereNotIn('order_group_id', ['def-order-group'])->whereNotIn('id', [$order['id']])->get();
    ...
    public function bulkOrderStatusChange(Request $request)
    {
        $order_ids = $request->input('order_ids');
        $order_status = $request->input('order_status');
         $value = Helpers::order_status_update_message($order_status);
            ...
            if ($value) {
                ...
                if (count($order_ids) > 0) {
                    foreach ($order_ids as $order_id) {
                        ...
                        OrderManager::stock_update_on_order_status_change($order, $order_status);
                      ...
                      $order->save();
                       ...
                       if ($request->order_status == 'delivered' && $order['seller_id'] != null) {
                        OrderManager::wallet_manage_on_order_status_change($order, 'admin');
                        OrderDetail::where('order_id', $order_id)->update(['delivery_status'=>'delivered']);
                    }
