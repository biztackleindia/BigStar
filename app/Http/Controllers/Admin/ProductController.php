...
class ProductController extends BaseController
{
   ...
   if( $request['discount']){
        ...
        if( $request['member_discount']){
            if ($request['member_discount_type'] == 'percent') {
                $mem_dis = ($request['unit_price'] / 100) * $request['member_discount'];
            } else {
                $mem_dis = $request['member_discount'];
            }
        }
        else{
            $mem_dis=0;
        }
        if ($request['unit_price'] <= $mem_dis) {
            $validator->after(function ($validator) {
                $validator->errors()->add(
                    'unit_price', 'Member discount can not be more or equal to the price!'
                );
            });
        }
        ...
        if($p->name ==''){
            $validator->after(function ($validator) {
                $validator->errors()->add('name', 'Product Name is required!');
            }); 
        }
        ...
        $p->member_discount_type = $request->member_discount_type;
        $p->member_discount = $request->member_discount_type == 'flat' ? BackEndHelper::currency_to_usd($request->member_discount) : $request->member_discount;
    ...
    public function update(Request $request, $id)
    {
     ...
     if($request['discount']){
        ...
        $product->member_discount_type = $request->member_discount_type;
        $product->member_discount = $request->member_discount_type == 'flat' ? BackEndHelper::currency_to_usd($request->member_discount) : $request->member_discount;
    ...
    public function bulk_import_data(Request $request)
    {
        ...
        $data = [];
        $skip = ['youtube_video_url', 'details', 'thumbnail','brand_id'];
        foreach ($collections as $collection) {
            ...
            array_push($data, [
                ...
                'tax' => $collection['tax'],
                'member_discount' => $collection['member_discount'],
                'member_discount_type' => $collection['member_discount_type'],
                'discount' => $collection['discount'],
                'discount_type' => $collection['discount_type'],
                'current_stock' => $collection['current_stock'],
                'details' => $collection['details'],
                'video_provider' => 'youtube',
                'unit_weight' => $collection['unit_weight'],
                'product_sku' => $collection['product_sku'],
    ...
    public function bulk_export_data()
    {
        ...
        foreach ($products as $item) {
            ...
            $storage[] = [
                ...
                'member_discount_type' => $item->member_discount_type,
