...
class CustomerController extends Controller
{
        ...
        if ($request->has('search')) {
            ...
            ->where(function ($q) use ($key) {
                foreach ($key as $value) {
                    $q->orWhere('member_id',null);
                }
            })
                ->where(function ($q) use ($key) {
                    foreach ($key as $value) {
                        $q->orWhere('f_name', 'like', "%{$value}%")->orWhere('l_name', 'like', "%{$value}%")->orWhere('phone', 'like', "%{$value}%")->orWhere('email', 'like', "%{$value}%");
                    }
                });
    ...
    public function member_list(Request $request)
    {
        ...
        if ($request->has('search')) {
            ...
            ->where(function ($q) use ($key) {
                ...
                ->where(function ($q) use ($key) {
                    foreach ($key as $value) {
                        $q->orWhere('f_name', 'like', "%{$value}%")->orWhere('l_name', 'like', "%{$value}%")->
                        orWhere('phone', 'like', "%{$value}%")->orWhere('email', 'like', "%{$value}%")
                        ;
                    }
                });
            $query_param = ['search' => $request['search']];
        ...
        return view('admin-views.member.list', compact('customers', 'search'));
    }
    ...
    public function Approvestatus_update(Request $request)
    {
        User::where(['id' => $request['id']])->update(['member_approval' => 1]);
        User::where(['id' => auth('customer')->id()])->update(['member_approval_status' => 0]);
        return response()->json([], 200);
    }
    public function RejectStatus_update(Request $request)
    {
        User::where(['id' => $request['id']])->update(['member_approval' => 2]);
        User::where(['id' => auth('customer')->id()])->update(['member_approval_status' => 0]);
        return response()->json([], 200);
    }
