...
class Helpers
{
    ...
    public static function WeightWithShippingCost($delivery_pin, $weight){
        $client = new Client(); 
        ...
        Toastr::warning(translate('Delivery not available for your pincode'));
        return 0;
        }
    ...
    public static function CheckingReferralCode($frd_ref_code){

        $user_reference_codes = user_reference_codes::where('reference_codes', $frd_ref_code)->first();
        if($user_reference_codes == null){
            $user_id =0 ;
        }
        else{
          $user_id =$user_reference_codes->user_id ;  
        }
    ...
    {
        $data = DB::table('pickup_locations')->where(['is_active' => 1])->get();
        return  $data;
    }
    ...
    public static function get_member_price_range($product)
    {
        $lowest_price = $product->unit_price;
        $highest_price = $product->unit_price;
        ...
        $lowest_price = Helpers::currency_converter($lowest_price - Helpers::get_member_product_discount($product, $lowest_price));
        $highest_price = Helpers::currency_converter($highest_price - Helpers::get_member_product_discount($product, $highest_price));
    ...
    public static function get_member_product_discount($product, $price)
    {
        $discount = 0;
        if($product->member_discount> 0 && $product->member_discount != ''){
            if ($product->member_discount_type == 'percent') {
                $discount = ($price * $product->member_discount) / 100;
            } elseif ($product->member_discount_type == 'flat') {
                $discount = $product->member_discount;
            }
        }
        else{
        if ($product->discount_type == 'percent') {
