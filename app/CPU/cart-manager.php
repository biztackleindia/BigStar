    ...
    public static function cart_to_db()
    {
        ...
        if (session()->has('offline_cart')) {
            ...
            foreach ($cart as $item) {
                $cart_already_exist = Cart::where(['product_id' => $item['product_id'], 'customer_id' => $user->id])->first();
                
                if ($cart_already_exist) {
                    $status = 1;
                    $product = Product::find($item['product_id']);
                    $count = count(json_decode($product->variation));
                    $new_qty = $item['quantity'] +  $cart_already_exist->quantity;
                    if ($count) {
                        
                        for ($i = 0; $i < $count; $i++) {
                            if (json_decode($product->variation)[$i]->type == $cart['variant']) {
                                if (json_decode($product->variation)[$i]->qty < $new_qty) {
                                    $status = 0;
                                    $qty = $cart['quantity'];
                                }
                            }
                        }
                    } else if ($product['current_stock'] < $new_qty) {
                        $status = 0;
                        $qty = $cart['quantity'];
                    }
                    if ($status) {
                        $cart_already_exist['quantity'] = $new_qty;
                        $cart_already_exist['shipping_cost'] =  CartManager::get_shipping_cost_for_product_category_wise($product,$new_qty);
                    }
                    
                    $cart_already_exist->save();
                }
                else{
    ...
    public static function add_to_cart($request, $from_api = false)
    {
        ...
        if(auth('customer')->check()){
            ...
            $product->member_discount > 0 && $product->member_discount != ''){
                $discount= Helpers::get_member_product_discount($product, $price);
            }
            else{
                $discount= Helpers::get_product_discount($product, $price);
            }
       }
        else{
            $discount= Helpers::get_product_discount($product, $price);
